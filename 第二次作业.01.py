...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    blend_lap = torch.nn.functional.conv2d(blended_img, laplacian_kernel, padding=1, groups=3)

    min_h = min(fg_lap.shape[2], blend_lap.shape[2])
    min_w = min(fg_lap.shape[3], blend_lap.shape[3])

    fg_lap = fg_lap[..., :min_h, :min_w]
    foreground_mask = foreground_mask[..., :min_h, :min_w]
    blend_lap = blend_lap[..., :min_h, :min_w]
    background_mask = background_mask[..., :min_h, :min_w]

    loss = (fg_lap * foreground_mask - blend_lap * background_mask).pow(2).mean()
    return loss
